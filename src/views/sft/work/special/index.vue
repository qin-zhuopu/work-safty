...
const rowClassName = ({ row }: { row: SpecialWorker }) => {
  return isExpiring(row.endTime, row.checkTime) ? "warning-row" : "";
};
...
async function fetchData(page = 1) {
  ...
  try {
    ...
    const response = await fetch(`/sft/work/special/list.json?${params}`);
...
function handleDownloadTemplate() {
  window.open("/pub/xls/特殊工模板（注意格式为2003）.xls");
}
  ...
  <div class="special-container">
    <el-card shadow="never">
      ...
      >
        ...
        <el-table-column prop="deptName" label="所属单位" width="120" />
        ...
        <el-table-column label="复审时间" width="150" align="center">
          <template #default="{ row }">
            {{ formatDate(row.checkTime) }}
          </template>
        </el-table-column>
        <el-table-column label="有效期至" width="150" align="center">
          <template #default="{ row }">
            {{ formatDate(row.endTime) }}
          </template>
        </el-table-column>
        <el-table-column prop="issuedDept" label="发证单位" width="140" />
