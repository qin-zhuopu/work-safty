...
async function fetchData() {
  ...
  try {
    ...
    const response = await fetch(
      `/sft/insp/approval/library/list.json?${params}`
    );
...
function getUserStatusType(status: string): "success" | "danger" | "info" {
    ...
    <el-card shadow="never">
      <template #header>
        <div class="card-header">
          <span class="text-lg font-medium">审批库管理</span>
        </div>
      </template>
      ...
      >
        ...
        <el-table-column label="人员状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag
              v-if="row.sysUser"
              :type="getUserStatusType(row.sysUser.status)"
            >
              {{ getUserStatusName(row.sysUser.status) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column
