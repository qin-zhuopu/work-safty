 ...
 * 测试巡检线路管理页面
 */
async function testTaskPage(page: Page): Promise<void> {
  console.log("\n=== 测试巡检线路管理页面 ===");

  // 导航到巡检线路管理页面
  await page.goto(`${BASE_URL}/sft/insp/task/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("巡检线路管理");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证搜索表单
  const searchInputs = page.locator(".search-form .el-input");
  ...
  const actionButtons = await page
    ...
    .locator(".el-button")
    .count();
  expect(actionButtons).toBeGreaterThanOrEqual(3);
  console.log(`✓ 操作列按钮数: ${actionButtons}`);
  ...
  await page.screenshot({
    path: "screenshots/sft-insp-task.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-insp-task.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/insp/task/index`);
    await page.goto(`${BASE_URL}/sft/insp/task/index`);
