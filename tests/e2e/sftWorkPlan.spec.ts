 ...
 * 测试施工方案数据库页面
 */
async function testWorkPlanPage(page: Page): Promise<void> {
  console.log("\n=== 测试施工方案数据库页面 ===");

  // 导航到目标页面
  await page.goto(`${BASE_URL}/sft/work/plan/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("施工方案数据库");
  console.log("✓ 页面标题正确");

  // 2. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder*="步骤"]');
  ...
  expect(headerText).toContain("项目名称");
  expect(headerText).toContain("步骤");
  expect(headerText).toContain("危险源");
  console.log("✓ 表头列正确");
  ...
  // 10. 验证L值/S值/R值列
  const lValueHeader = await page.locator("th:has-text('L值')").count();
  const sValueHeader = await page.locator("th:has-text('S值')").count();
  const rValueHeader = await page.locator("th:has-text('R值')").count();
  if (lValueHeader + sValueHeader + rValueHeader > 0) {
    console.log("✓ 风险评估列(L/S/R值)存在");
  }

  // 11. 截图
  await page.screenshot({
    path: "screenshots/sft-work-plan.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-plan.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/plan/index`);
    await page.goto(`${BASE_URL}/sft/work/plan/index`);
