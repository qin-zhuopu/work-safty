...
async function connectToChrome(port: number = DEFAULT_CDP_PORT) {
  ...
  try {
    ...
    throw new Error(`无法连接到 Chrome CDP (${cdcUrl})`);
  }
}
...
async function testHmdPage(page: Page): Promise<void> {
  ...
  // 导航到页面
  await page.goto(`${BASE_URL}/sft/cont/manage/hmd/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("承包商黑名单");
  console.log("✓ 页面标题正确");
  ...
  expect(headerText).toContain("单位");
  expect(headerText).toContain("统一社会信用代码");
  expect(headerText).toContain("状态");
  expect(headerText).toContain("备注");
  ...
  const actionButtons = await page
    .locator(".el-table__body .el-table__row")
    .first()
    .locator("td:last-child .el-button")
    .allTextContents();
  const buttonLabels = actionButtons.map(b => b.trim());
  expect(buttonLabels).toContain("移出");
  expect(buttonLabels).toContain("违章记录");
  expect(buttonLabels).toContain("文件");
  expect(buttonLabels).toContain("人员");
  console.log("✓ 操作按钮正确");

  // 10. 测试搜索功能
  await searchInput.fill("淮安");
  ...
  await page.screenshot({
    path: "screenshots/sft-cont-manage-hmd.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-cont-manage-hmd.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/cont/manage/hmd/index`);
    await page.goto(`${BASE_URL}/sft/cont/manage/hmd/index`);
