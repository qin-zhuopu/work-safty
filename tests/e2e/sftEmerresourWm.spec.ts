 ...
 * 测试排班管理页面
 */
async function testWmPage(page: Page): Promise<void> {
  console.log("\n=== 测试排班管理页面 ===");

  // 导航到排班管理页面
  await page.goto(`${BASE_URL}/sft/emerresour/wm/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("排班管理");
  console.log("✓ 页面标题正确");
  ...
  // 5. 验证搜索框
  const searchInput = page.locator('input[placeholder*="值班名称"]');
  ...
  console.log("✓ 表格操作按钮存在");

  // 8. 截图
  await page.screenshot({
    path: "screenshots/sft-emerresour-wm.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-emerresour-wm.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/emerresour/wm/index`);
    await page.goto(`${BASE_URL}/sft/emerresour/wm/index`);
