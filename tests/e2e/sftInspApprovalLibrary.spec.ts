...
async function testApprovalLibraryPage(page: Page): Promise<void> {
  ...
  // 导航到页面
  await page.goto(`${BASE_URL}/sft/insp/approval/library/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("审批库管理");
  console.log("✓ 页面标题正确");
  ...
  expect(headerText).toContain("人员");
  expect(headerText).toContain("账号");
  expect(headerText).toContain("人员状态");
  expect(headerText).toContain("类型");
  expect(headerText).toContain("审批权限");
  console.log("✓ 表头正确");
  ...
  const nameInput = page.locator('input[placeholder*="请输入审批人员"]');
  ...
  console.log(`✓ 人员状态标签数: ${tagCount}`);

  // 8. 测试搜索功能 - 按类型
  await typeSelect.first().click();
  await page.waitForTimeout(300);
  const typeOption = page
    .locator(".el-select-dropdown__item:has-text('管信审批人')")
    .first();
  ...
  await page.screenshot({
    path: "screenshots/sft-insp-approval-library.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-insp-approval-library.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/insp/approval/library/index`);
    await page.goto(`${BASE_URL}/sft/insp/approval/library/index`);
