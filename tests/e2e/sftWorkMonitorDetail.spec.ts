 ...
 * 测试特殊作业监控设备管理页面
 */
async function testWorkMonitorDetailPage(page: Page): Promise<void> {
  console.log("\n=== 测试特殊作业监控设备管理页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/work/monitor/detail`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("特殊作业监控设备管理");
  console.log("✓ 页面标题正确");
  ...
  console.log("表头列:", headers.filter(h => h.trim()).slice(0, 4));
  ...
  const firstRowCells = await page
    ...
    .locator("td")
    .allTextContents();
  console.log("第一行数据:", firstRowCells.slice(1, 4).join(" | "));

  // 6. 验证搜索功能
  await page.fill('input[placeholder*="名称"]', "实华监控设备1");
  await page.click('button:has-text("检索")');
  await page.waitForTimeout(500);
  console.log("✓ 检索功能测试完成");
  ...
  const videoButtons = page.locator('button:has-text("视频列表")');
  const videoButtonCount = await videoButtons.count();
  expect(videoButtonCount).toBeGreaterThan(0);
  console.log(`✓ 视频列表按钮数: ${videoButtonCount}`);
  ...
  await page.screenshot({
    path: "screenshots/sft-work-monitor-detail.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-monitor-detail.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/monitor/detail`);
    await page.goto(`${BASE_URL}/sft/work/monitor/detail`);
    ...
    // 运行测试
    await testWorkMonitorDetailPage(page);
  } catch (err: any) {
