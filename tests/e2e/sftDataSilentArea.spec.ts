 ...
 * 测试静默区域页面
 */
async function testSilentAreaPage(page: Page): Promise<void> {
  console.log("\n=== 测试静默区域页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/data/silent/area/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("静默区域");
  console.log("✓ 页面标题正确");
  ...
  expect(headerText).toContain("计划开始时间");
  expect(headerText).toContain("计划结束时间");
  ...
  const addBtn = page.locator("button:has-text('新建')");
  await expect(searchBtn.first()).toBeVisible();
  await expect(resetBtn.first()).toBeVisible();
  await expect(addBtn.first()).toBeVisible();
  console.log("✓ 检索/重置/新建按钮存在");

  // 7. 验证表格内操作按钮
  const editBtn = page.locator(".el-table__body button:has-text('编辑'), .el-table__body .el-button--small:not(.el-button--link)");
  const deleteBtn = page.locator(".el-table__body button:has-text('删除'), .el-table__body .el-button--link");
  const buttonCount = await page.locator(".el-table__body button").count();
  expect(buttonCount).toBeGreaterThan(0);
  console.log(`✓ 操作按钮存在 (共 ${buttonCount} 个)`);

  // 8. 测试搜索功能
  await searchInput.fill("生产");
  ...
  await page.screenshot({
    path: "screenshots/sft-data-silent-area.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-data-silent-area.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/data/silent/area/index`);
    await page.goto(`${BASE_URL}/sft/data/silent/area/index`);
