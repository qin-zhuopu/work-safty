 ...
 * 测试行为安全观察页面
 */
async function testBehaviorSafePage(page: Page): Promise<void> {
  console.log("\n=== 测试行为安全观察与沟通管理页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/behavior/safe`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("行为安全观察与沟通管理");
  console.log("✓ 页面标题正确");
  ...
  // 7. 测试搜索功能 - 观察人搜索
  await page.fill('input[placeholder*="观察人"]', "张三");
  ...
  // 9. 测试部门选择器
  const deptCascader = page.locator(".el-cascader").first();
  await expect(deptCascader).toBeVisible();
  console.log("✓ 部门选择器存在");
  ...
  await page.screenshot({
    path: "screenshots/sft-behavior-safe.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-behavior-safe.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/behavior/safe`);
    await page.goto(`${BASE_URL}/sft/behavior/safe`);
    ...
    // 运行测试
    await testBehaviorSafePage(page);
  } catch (err: any) {
