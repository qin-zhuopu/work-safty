 ...
 * 测试承包商人员黑名单页面
 */
async function testHmdPage(page: Page): Promise<void> {
  console.log("\n=== 测试承包商人员黑名单页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/cont/per/hmd/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("承包商人员黑名单");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证身份证号输入框
  const idCardInput = page.locator('input[placeholder*="请输入身份证号"]');
  await expect(idCardInput.first()).toBeVisible();
  console.log("✓ 身份证号输入框存在");

  // 4. 验证承包商名称输入框
  const contractorInput = page.locator(
    'input[placeholder*="请输入承包商名称"]'
  );
  await expect(contractorInput.first()).toBeVisible();
  console.log("✓ 承包商名称输入框存在");
  ...
  expect(headers.some(h => h.includes("身份证号"))).toBeTruthy();
  console.log("✓ 表头正确");
  ...
  // 10. 验证操作列存在移出按钮
  const removeButtons = page.locator(".el-button--danger:has-text('移出')");
  const removeButtonCount = await removeButtons.count();
  expect(removeButtonCount).toBeGreaterThan(0);
  console.log(`✓ 移出按钮数: ${removeButtonCount}`);

  // 11. 测试搜索功能
  await nameInput.fill("管金峰");
  ...
  await page.screenshot({
    path: "screenshots/sft-cont-per-hmd.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-cont-per-hmd.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/cont/per/hmd/index`);
    await page.goto(`${BASE_URL}/sft/cont/per/hmd/index`);
