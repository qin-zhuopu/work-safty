 ...
 * 测试奖惩记录页面
 */
async function testRewardsRecordPage(page: Page): Promise<void> {
  console.log("\n=== 测试奖惩记录页面 ===");

  // 导航到奖惩记录页面
  await page.goto(`${BASE_URL}/sft/hd/rewards/record/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("奖惩管理");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证搜索表单
  const hdNameInput = page.locator('input[placeholder="请输入隐患名称"]');
  const userNameInput = page.locator('input[placeholder="请输入奖惩人员"]');
  await expect(hdNameInput).toBeVisible();
  ...
  if (dataRows === 0) {
    console.error("\n⚠ 表格没有数据行！");
    console.error("这可能是因为 Mock API 没有被正确加载。");
    console.error("请尝试以下步骤：");
    console.error("1. 停止当前的开发服务器 (Ctrl+C)");
    console.error("2. 重新运行: pnpm dev");
    console.error("3. 重新运行测试: pnpm test:e2e:sft:hd:rewards:record\n");
    throw new Error("表格没有数据，请检查 Mock API 是否正确配置");
  }
  ...
  await page.waitForTimeout(500);
  const hdNameValue = await hdNameInput.inputValue();
  expect(hdNameValue).toBe("");
  console.log("✓ 重置功能已测试");
  ...
  await page.screenshot({
    path: "screenshots/sft-hd-rewards-record.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-hd-rewards-record.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/hd/rewards/record/index`);
    await page.goto(`${BASE_URL}/sft/hd/rewards/record/index`);
    ...
    // 运行测试
    await testRewardsRecordPage(page);
  } catch (err: any) {
