 ...
 * 测试管理制度页面
 */
async function testManagePage(page: Page): Promise<void> {
  console.log("\n=== 测试管理制度页面 ===");

  // 导航到管理制度页面
  await page.goto(`${BASE_URL}/sft/danger/manage/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("管理制度");
  console.log("✓ 页面标题正确");
  ...
  expect(headers.some(h => h.includes("文件名称"))).toBeTruthy();
  expect(headers.some(h => h.includes("发布单位"))).toBeTruthy();
  expect(headers.some(h => h.includes("资料附件"))).toBeTruthy();
  expect(headers.some(h => h.includes("上传时间"))).toBeTruthy();
  ...
  const buttons = await page
    .locator(".search-form .el-button")
    .allTextContents();
  expect(buttons.some(b => b.includes("查询"))).toBeTruthy();
  expect(buttons.some(b => b.includes("重置"))).toBeTruthy();
  expect(buttons.some(b => b.includes("添加"))).toBeTruthy();
  expect(buttons.some(b => b.includes("删除"))).toBeTruthy();
  console.log("✓ 操作按钮存在");

  // 8. 测试搜索功能
  await nameInput.first().fill("隐患");
  await page.click(".search-form .el-button:has-text('查询')");
  await page.waitForTimeout(500);
  console.log("✓ 搜索功能测试完成");

  // 9. 测试重置功能
  await page.click(".search-form .el-button:has-text('重置')");
  await page.waitForTimeout(500);
  ...
  await page.screenshot({
    path: "screenshots/sft-danger-manage.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-danger-manage.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/danger/manage/index`);
    await page.goto(`${BASE_URL}/sft/danger/manage/index`);
