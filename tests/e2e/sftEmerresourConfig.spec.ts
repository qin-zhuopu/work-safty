 ...
 * 测试维护记录配置台账页面
 */
async function testConfigPage(page: Page): Promise<void> {
  console.log("\n=== 测试维护记录配置台账页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/emerresour/config/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("维护记录配置台账");
  console.log("✓ 页面标题正确");

  // 2. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder*="请输入类型"]');
  await expect(searchInput.first()).toBeVisible();
  console.log("✓ 搜索表单已渲染");

  // 3. 验证操作按钮存在
  const buttons = ["查询", "重置", "添加", "下载模板", "导入", "删除"];
  ...
  // 8. 验证操作列
  const actionButtons = page.locator(".el-table__body .el-button.is-link");
  ...
  // 9. 测试搜索功能
  console.log("\n测试搜索功能...");
  await searchInput.first().fill("空气呼吸器");
  ...
  await page.screenshot({
    path: "screenshots/sft-emerresour-config.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-emerresour-config.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/emerresour/config/index`);
    await page.goto(`${BASE_URL}/sft/emerresour/config/index`);
