 ...
 * 测试应急物资台账页面
 */
async function testMaterialPage(page: Page): Promise<void> {
  console.log("\n=== 测试应急物资台账页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/emerresour/material/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("应急物资台账");
  console.log("✓ 页面标题正确");
  ...
  // 6. 验证搜索表单元素
  const searchFormExists = await page.locator(".search-form").count();
  expect(searchFormExists).toBeGreaterThan(0);
  console.log("✓ 搜索表单存在");
  ...
  const buttons = await page
    ...
    .allTextContents();
  console.log("操作按钮:", buttons.filter(b => b.trim()).slice(0, 6));

  // 8. 验证分页器
  const paginationExists = await page.locator(".el-pagination").count();
  expect(paginationExists).toBeGreaterThan(0);
  console.log("✓ 分页器存在");

  // 9. 验证警告行样式（没有维护日期的行）
  const warningRows = await page.locator(".row-warning").count();
  console.log(`✓ 警告行数（无维护日期）: ${warningRows}`);

  // 10. 截图
  await page.screenshot({
    path: "screenshots/sft-emerresour-material.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-emerresour-material.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/emerresour/material/index`);
    await page.goto(`${BASE_URL}/sft/emerresour/material/index`);
