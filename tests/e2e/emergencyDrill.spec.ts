 ...
 * 测试演练计划管理页面
 */
async function testDrillPage(page: Page): Promise<void> {
  console.log("\n=== 测试演练计划管理页面 ===");

  // 导航到演练计划管理页面
  await page.goto(`${BASE_URL}/emergency/drill/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const pageTitle = await page.title();
  expect(pageTitle).toContain("演练计划管理");
  console.log("✓ 页面标题正确");
  ...
  const downloadButton = page.locator("button:has-text('下载模板')");
  await expect(downloadButton.first()).toBeVisible();
  console.log("✓ 下载模板按钮存在");
  ...
  // 7. 验证表格列中的操作按钮
  const editButtons = page.locator(".el-button:has-text('编辑')");
  ...
  await page.screenshot({
    path: "screenshots/emergency-drill-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/emergency-drill-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/emergency/drill/index`);
    await page.goto(`${BASE_URL}/emergency/drill/index`);
