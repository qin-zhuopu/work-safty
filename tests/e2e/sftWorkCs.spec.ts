 ...
 * 测试施工方案页面
 */
async function testWorkCsPage(page: Page): Promise<void> {
  console.log("\n=== 测试施工方案页面 ===");

  // 导航到目标页面
  await page.goto(`${BASE_URL}/sft/work/cs/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("施工方案");
  console.log("✓ 页面标题正确");

  // 2. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder*="项目名称"]');
  ...
  expect(headerText).toContain("申请时间");
  expect(headerText).toContain("施工单位");
  console.log("✓ 表头列正确");
  ...
  const detailButtons = page.locator(
    '.el-table__body .el-button--primary:has-text("详情")'
  );
  ...
  console.log(`✓ 详情按钮数: ${detailCount}, 删除按钮数: ${deleteCount}`);
  ...
  await page.screenshot({
    path: "screenshots/sft-work-cs.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-cs.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/cs/index`);
    await page.goto(`${BASE_URL}/sft/work/cs/index`);
