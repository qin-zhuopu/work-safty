 ...
 * 测试变更管理页面
 */
async function testChangeChangePage(page: Page): Promise<void> {
  console.log("\n=== 测试变更管理页面 ===");

  // 导航到变更管理页面
  await page.goto(`${BASE_URL}/sft/change/change/index`);
  ...
  const hasSearchInput =
    (await page.locator(searchInputs[0]).count()) > 0 ||
    (await page.locator(searchInputs[1]).count()) > 0;
  expect(hasSearchInput).toBe(true);
  console.log("✓ 搜索表单已渲染");

  // 3. 验证功能按钮存在
  const buttons = ["查询", "重置", "添加", "导出"];
  ...
  expect(headerText).toContain("变更项目名称");
  expect(headerText).toContain("变更类型");
  console.log("✓ 表头列正确");
  ...
  await page.screenshot({
    path: "screenshots/sft-change-change.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-change-change.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/change/change/index`);
    await page.goto(`${BASE_URL}/sft/change/change/index`);
    ...
    // 运行测试
    await testChangeChangePage(page);
  } catch (err: any) {
