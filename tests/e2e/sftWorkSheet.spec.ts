 ...
 * 测试派工单管理页面
 */
async function testSheetPage(page: Page): Promise<void> {
  console.log("\n=== 测试派工单管理页面 ===");

  // 导航到派工单管理页面
  await page.goto(`${BASE_URL}/sft/work/sheet/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("派工单管理");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证搜索表单字段
  const deptSelect = page.locator(
    '.el-cascader, input[placeholder*="申请部门"]'
  );
  await expect(deptSelect.first()).toBeVisible();
  console.log("✓ 申请部门选择框存在");

  // 4. 验证状态选择器
  const statusSelect = page.locator("select, .el-select");
  ...
  const queryBtn = page.locator(
    'button:has-text("查询"), button:has-text("检索")'
  );
  ...
  expect(headerText).toContain("申请人");
  expect(headerText).toContain("申请单位");
  expect(headerText).toContain("作业内容");
  console.log("✓ 关键表头列存在");
  ...
  // 9. 验证操作列按钮
  const approveBtns = page.locator('button:has-text("审批流")');
  const approveCount = await approveBtns.count();
  expect(approveCount).toBeGreaterThan(0);
  console.log(`✓ 审批流按钮数: ${approveCount}`);
  ...
  await page.screenshot({
    path: "screenshots/sft-work-sheet.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-sheet.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/sheet/index`);
    await page.goto(`${BASE_URL}/sft/work/sheet/index`);
