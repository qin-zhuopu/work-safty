 ...
 * 测试巡检设备清单管理页面
 */
async function testDevicePage(page: Page): Promise<void> {
  console.log("\n=== 测试巡检设备清单管理页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/insp/device/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("巡检设备清单管理");
  console.log("✓ 页面标题正确");
  ...
  expect(headerText).toContain("巡检点名称");
  expect(headerText).toContain("设备编号");
  expect(headerText).toContain("所属车间");
  console.log("✓ 表头正确");
  ...
  // 5. 验证搜索框
  const deptNameInput = page.locator('input[placeholder*="车间名称"]');
  const nameInput = page.locator('input[placeholder*="巡检点名称"]');
  const abcInput = page.locator('input[placeholder*="ABC编号"]');
  await expect(deptNameInput.first()).toBeVisible();
  await expect(nameInput.first()).toBeVisible();
  await expect(abcInput.first()).toBeVisible();
  console.log("✓ 搜索框存在");
  ...
  // 验证其他操作按钮存在
  const _downloadBtn = page.locator("button:has-text('下载模板')");
  const _importBtn = page.locator("button:has-text('导入')");
  const _exportBtn = page.locator("button:has-text('导出')");
  const _deleteBtn = page.locator("button:has-text('删除')");
  console.log("✓ 操作按钮存在");
  ...
  // 8. 验证选择框
  const checkboxColumn = page.locator(".el-table__body .el-checkbox");
  expect(await checkboxColumn.count()).toBeGreaterThan(0);
  console.log("✓ 选择框存在");

  // 9. 测试搜索功能
  await nameInput.fill("反应釜");
  ...
  console.log(`✓ 设备状态标签数: ${tagCount}`);

  // 13. 截图
  await page.screenshot({
    path: "screenshots/sft-insp-device.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-insp-device.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/insp/device/index`);
    await page.goto(`${BASE_URL}/sft/insp/device/index`);
