 ...
 * 测试管道设备监测页面
 */
async function testGdsPage(page: Page): Promise<void> {
  console.log("\n=== 测试管道设备监测页面 ===");

  // 导航到目标页面
  await page.goto(`${BASE_URL}/data/gds/index2`);
  ...
  const timeCell = await page
    ...
    .nth(4)
    .textContent();
  expect(timeCell).toMatch(/\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}/);
  console.log("✓ 时间格式正确");

  // 8. 截图
  await page.screenshot({
    path: "screenshots/data-gds-index2.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/data-gds-index2.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/data/gds/index2`);
    await page.goto(`${BASE_URL}/data/gds/index2`);
