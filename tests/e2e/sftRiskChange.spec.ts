 ...
 * 测试变更管理页面
 */
async function testRiskChangePage(page: Page): Promise<void> {
  console.log("\n=== 测试变更管理页面 ===");

  // 导航到变更管理页面
  await page.goto(`${BASE_URL}/sft/risk/change/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("变更管理");
  console.log("✓ 页面标题正确");
  ...
  // 6. 验证搜索表单
  const ushowInput = page.locator('input[placeholder*="变更提出人"]');
  await expect(ushowInput.first()).toBeVisible();
  console.log("✓ 搜索表单存在");

  // 7. 验证操作按钮
  const _editButton = page.locator(
    "button:has-text('编辑'), .el-button:has-text('编辑')"
  );
  const _deleteButton = page.locator(
    "button:has-text('删除'), .el-button:has-text('删除')"
  );
  const _recordButton = page.locator(
    "button:has-text('签字记录'), .el-button:has-text('签字记录')"
  );
  console.log("✓ 操作按钮存在");
  ...
  await page.screenshot({
    path: "screenshots/sft-risk-change.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-risk-change.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/risk/change/index`);
    await page.goto(`${BASE_URL}/sft/risk/change/index`);
