...
async function testSpecialPage(page: Page): Promise<void> {
  ...
  // 导航到专项检查任务页面
  await page.goto(`${BASE_URL}/sft/risk/special/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("专项检查任务");
  console.log("✓ 页面标题正确");
  ...
  // 5. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder="任务名称"]');
  ...
  await page.screenshot({
    path: "screenshots/sft-risk-special.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-risk-special.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/risk/special/index`);
    await page.goto(`${BASE_URL}/sft/risk/special/index`);
