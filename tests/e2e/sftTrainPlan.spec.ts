...
async function testPlanPage(page: Page): Promise<void> {
  ...
  // 导航到页面
  await page.goto(`${BASE_URL}/sft/train/plan/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("计划管理");
  console.log("✓ 页面标题正确");

  // 2. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder*="计划名称"]');
  await expect(searchInput.first()).toBeVisible();
  console.log("✓ 搜索框存在");

  // 3. 验证培训方式选择框
  const methodSelect = page.locator(".search-form .el-select").first();
  await expect(methodSelect).toBeVisible();
  console.log("✓ 培训方式选择框存在");
  ...
  // 10. 验证操作按钮存在
  const actionBtns = page.locator(".el-table__body .el-button");
  ...
  await page.screenshot({
    path: "screenshots/sft-train-plan.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-train-plan.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/train/plan/index`);
    await page.goto(`${BASE_URL}/sft/train/plan/index`);
