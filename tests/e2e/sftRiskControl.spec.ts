 ...
 * 测试风险分级管控页面
 */
async function testRiskControlPage(page: Page): Promise<void> {
  console.log("\n=== 测试风险分级管控页面 ===");

  // 导航到风险分级管控页面
  await page.goto(`${BASE_URL}/sft/risk/control/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("风险分级管控");
  console.log("✓ 页面标题正确");

  // 2. 验证搜索表单存在
  const searchForm = page.locator(".search-form, .el-form");
  await expect(searchForm.first()).toBeVisible();
  console.log("✓ 搜索表单已渲染");

  // 3. 验证搜索字段
  const searchInputs = await page
    .locator(".el-form-item label")
    .allTextContents();
  console.log("搜索字段:", searchInputs.slice(0, 5));
  expect(searchInputs.some(text => text.includes("风险分析对象"))).toBeTruthy();
  expect(
    searchInputs.some(text => text.includes("风险分析单元名称"))
  ).toBeTruthy();
  ...
  expect(headers.some(h => h.includes("风险分析单元名称"))).toBeTruthy();
  expect(headers.some(h => h.includes("风险等级"))).toBeTruthy();
  ...
  // 7. 验证风险等级标签（应该有不同颜色的标签）
  const levelTags = page.locator(".el-tag");
  const tagCount = await levelTags.count();
  expect(tagCount).toBeGreaterThan(0);
  console.log(`✓ 风险等级标签数: ${tagCount}`);

  // 8. 验证操作按钮（编辑、删除、管控任务、选点）
  const editButtons = page.locator(
    "a:has-text('编辑'), button:has-text('编辑')"
  );
  const editCount = await editButtons.count();
  expect(editCount).toBeGreaterThan(0);
  console.log(`✓ 操作按钮数: ${editCount}`);

  // 9. 验证分页组件
  const pagination = page.locator(".el-pagination");
  const paginationVisible = await pagination.count();
  ...
  await page.screenshot({
    path: "screenshots/risk-control-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/risk-control-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/risk/control/index`);
    await page.goto(`${BASE_URL}/sft/risk/control/index`);
    ...
    // 运行测试
    await testRiskControlPage(page);
  } catch (err: any) {
