 ...
 * 测试题库管理页面
 */
async function testItembankPage(page: Page): Promise<void> {
  console.log("\n=== 测试题库管理页面 ===");

  // 导航到题库管理页面
  await page.goto(`${BASE_URL}/sft/train/itembank/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("题库管理");
  console.log("✓ 页面标题正确");
  ...
  // 8. 验证筛选条件
  const typeSelect = page.locator(".search-form .el-select");
  await expect(typeSelect.first()).toBeVisible();
  console.log("✓ 类型选择框存在");

  const titleInput = page.locator('.search-form input[placeholder*="标题"]');
  await expect(titleInput.first()).toBeVisible();
  console.log("✓ 标题输入框存在");

  // 9. 测试搜索功能
  await page.fill('.search-form input[placeholder*="标题"]', "AI");
  ...
  await page.screenshot({
    path: "screenshots/sft-train-itembank.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-train-itembank.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/train/itembank/index`);
    await page.goto(`${BASE_URL}/sft/train/itembank/index`);
