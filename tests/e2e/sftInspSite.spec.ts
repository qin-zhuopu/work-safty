 ...
 * 测试巡检点管理页面
 */
async function testInspSitePage(page: Page): Promise<void> {
  console.log("\n=== 测试巡检点管理页面 ===");

  // 导航到巡检点管理页面
  await page.goto(`${BASE_URL}/sft/insp/site/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("巡检点管理");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证表单筛选条件
  const nameLabel = await page.locator('label:has-text("巡检点名称")').count();
  const workLabel = await page.locator('label:has-text("所属工段")').count();
  const userLabel = await page
    .locator('label:has-text("工段管理人员")')
    .count();
  expect(nameLabel + workLabel + userLabel).toBeGreaterThan(0);
  console.log("✓ 筛选条件存在");

  // 4. 验证表头
  const headers = await page.locator(".el-table__header th").allTextContents();
  console.log("表头列:", headers.filter(h => h.trim()).slice(0, 6));
  expect(headers.some(h => h.includes("巡检点名称"))).toBeTruthy();
  expect(headers.some(h => h.includes("巡检编码"))).toBeTruthy();
  ...
  expect(editButtons + deleteButtons).toBeGreaterThan(0);
  console.log("✓ 操作按钮存在");

  // 8. 验证顶部功能按钮
  const searchButton = await page.locator('button:has-text("查询")').count();
  const resetButton = await page.locator('button:has-text("重置")').count();
  const addButton = await page.locator('button:has-text("添加")').count();
  expect(searchButton + resetButton + addButton).toBeGreaterThan(0);
  console.log("✓ 功能按钮存在");

  // 9. 测试搜索功能
  await page.fill('input[placeholder*="巡检点名称"]', "液氯");
  ...
  await page.screenshot({
    path: "screenshots/sft-insp-site.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-insp-site.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/insp/site/index`);
    await page.goto(`${BASE_URL}/sft/insp/site/index`);
