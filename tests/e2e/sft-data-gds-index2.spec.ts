 ...
 * 测试管道设备监测数据页面
 */
async function testDataGdsIndex2Page(page: Page): Promise<void> {
  console.log("\n=== 测试管道设备监测数据页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/data/gds/index2`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("管道设备监测数据");
  console.log("✓ 页面标题正确");
  ...
  expect(headers.some(h => h.includes("设备类型"))).toBeTruthy();
  expect(headers.some(h => h.includes("设备名称"))).toBeTruthy();
  ...
  await page.screenshot({
    path: "screenshots/sft-data-gds-index2.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-data-gds-index2.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/data/gds/index2`);
    await page.goto(`${BASE_URL}/sft/data/gds/index2`);
    ...
    // 运行测试
    await testDataGdsIndex2Page(page);
  } catch (err: any) {
