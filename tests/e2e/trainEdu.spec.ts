...
async function testEduPage(page: Page): Promise<void> {
  ...
  // 导航到教育培训管理页面
  await page.goto(`${BASE_URL}/train/edu/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("教育培训管理");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder="请输入标题"]');
  ...
  expect(headerText).toContain("标题");
  ...
  await page.screenshot({
    path: "screenshots/train-edu-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/train-edu-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/train/edu/index`);
    await page.goto(`${BASE_URL}/train/edu/index`);
