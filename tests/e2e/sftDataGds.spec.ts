...
async function testGdsPage(page: Page): Promise<void> {
  ...
  // 导航到监测报警数据页面
  await page.goto(`${BASE_URL}/sft/data/gds/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("重大危险源监测报警数据");
  console.log("✓ 页面标题正确");
  ...
  await page.screenshot({
    path: "screenshots/sft-data-gds.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-data-gds.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/data/gds/index`);
    await page.goto(`${BASE_URL}/sft/data/gds/index`);
