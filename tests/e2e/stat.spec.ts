 ...
 * 测试统计页面
 */
async function testStatPage(page: Page): Promise<void> {
  console.log("\n=== 测试统计页面 ===");

  // 导航到统计页面
  await page.goto(`${BASE_URL}/resum/plan/stat`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("重大危险源包保责任制履职排查计划统计");
  console.log("✓ 页面标题正确");
  ...
  // 6. 验证完成率标签（应该有不同颜色的标签）
  const rateTags = page.locator(".el-tag");
  const tagCount = await rateTags.count();
  expect(tagCount).toBeGreaterThan(0);
  console.log(`✓ 完成率标签数: ${tagCount}`);

  // 7. 验证嵌套表头（三级负责人）
  const mainHeader = await page.locator("th:has-text('主要负责人')").count();
  const techHeader = await page.locator("th:has-text('技术负责人')").count();
  const operHeader = await page.locator("th:has-text('操作负责人')").count();
  if (mainHeader + techHeader + operHeader > 0) {
    console.log("✓ 嵌套表头存在");
  } else {
    console.log("⚠ 嵌套表头未找到（可能是旧系统表格）");
  }

  // 8. 截图
  await page.screenshot({ path: "screenshots/stat-page.png", fullPage: true });
  console.log("✓ 已保存截图: screenshots/stat-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/resum/plan/stat`);
    await page.goto(`${BASE_URL}/resum/plan/stat`);
