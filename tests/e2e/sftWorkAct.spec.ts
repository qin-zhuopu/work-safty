 ...
 * 测试作业活动页面
 */
async function testActPage(page: Page): Promise<void> {
  console.log("\n=== 测试作业活动页面 ===");

  // 导航到作业活动页面
  await page.goto(`${BASE_URL}/sft/work/act/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("作业活动");
  console.log("✓ 页面标题正确");
  ...
  await page.screenshot({
    path: "screenshots/sft-work-act-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-act-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/act/index`);
    await page.goto(`${BASE_URL}/sft/work/act/index`);
