 ...
 * 测试模拟答题记录页面
 */
async function testMockRecordPage(page: Page): Promise<void> {
  console.log("\n=== 测试模拟答题记录页面 ===");

  // 导航到模拟答题记录页面
  await page.goto(`${BASE_URL}/train/pc/mock/record/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证错题总计卡片
  const errCountElement = page.locator(".error-count .value");
  await expect(errCountElement.first()).toBeVisible();
  const errCountText = await errCountElement.first().textContent();
  console.log(`✓ 错题总计: ${errCountText}`);

  // 2. 验证页面标题
  const title = await page.textContent(".card-header span");
  expect(title).toContain("模拟答题记录");
  console.log("✓ 页面标题正确");
  ...
  const dateCell = await page
    ...
    .nth(4)
    .textContent();
  expect(dateCell).toMatch(/\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}/);
  console.log(`✓ 日期格式正确: ${dateCell}`);

  // 9. 截图
  await page.screenshot({
    path: "screenshots/train-pc-mock-record-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/train-pc-mock-record-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/train/pc/mock/record/index`);
    await page.goto(`${BASE_URL}/train/pc/mock/record/index`);
