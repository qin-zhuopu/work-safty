 ...
 * 测试角色管理页面
 */
async function testRolePage(page: Page): Promise<void> {
  console.log("\n=== 测试角色管理页面 ===");

  // 导航到角色管理页面
  await page.goto(`${BASE_URL}/sys/role/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("角色管理");
  console.log("✓ 页面标题正确");
  ...
  const addButton = page.locator(
    '.card-header .el-button--primary:has-text("添加")'
  );
  ...
  if (isPaginationVisible) {
    ...
    console.log("⚠ 分页组件未显示（数据不足一页）");
  }

  // 10. 截图
  await page.screenshot({
    path: "screenshots/sys-role-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sys-role-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sys/role/index`);
    await page.goto(`${BASE_URL}/sys/role/index`);
