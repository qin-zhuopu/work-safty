 ...
 * 测试视频报警管理页面
 */
async function testAlarm2Page(page: Page): Promise<void> {
  console.log("\n=== 测试视频报警管理页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/data/alarm2/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("视频报警管理");
  console.log("✓ 页面标题正确");
  ...
  await page.screenshot({
    path: "screenshots/alarm2-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/alarm2-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/data/alarm2/index`);
    await page.goto(`${BASE_URL}/sft/data/alarm2/index`);
