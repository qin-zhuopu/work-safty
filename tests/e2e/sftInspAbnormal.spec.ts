 ...
 * 测试更换台账页面
 */
async function testAbnormalPage(page: Page): Promise<void> {
  console.log("\n=== 测试更换台账页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/insp/abnormal/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("更换台账");
  console.log("✓ 页面标题正确");
  ...
  expect(headerText).toContain("异常原因");
  expect(headerText).toContain("审批节点");
  expect(headerText).toContain("审批状态");
  console.log("✓ 表头正确");
  ...
  // 5. 验证搜索框
  const searchInput = page.locator('input[placeholder*="异常原因"]');
  ...
  // 7. 验证表格内操作按钮
  const siteBtn = page.locator("button:has-text('巡检点')");
  const approveBtn = page.locator(
    "button.el-button--danger:has-text('审批流')"
  );
  const deleteBtn = page.locator("button.el-button--link:has-text('删除')");
  expect(await siteBtn.count()).toBeGreaterThan(0);
  ...
  await page.screenshot({
    path: "screenshots/sft-insp-abnormal.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-insp-abnormal.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/insp/abnormal/index`);
    await page.goto(`${BASE_URL}/sft/insp/abnormal/index`);
