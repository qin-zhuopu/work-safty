 ...
 * 测试作业管理页面
 */
async function testWorkDocPage(page: Page): Promise<void> {
  console.log("\n=== 测试作业管理页面 ===");

  // 导航到作业管理页面
  await page.goto(`${BASE_URL}/sft/work/doc/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("作业管理");
  console.log("✓ 页面标题正确");
  ...
  // 7. 验证搜索表单
  const deptSelect = page.locator('select[placeholder*="部门"], .el-select');
  await expect(deptSelect.first()).toBeVisible();
  console.log("✓ 搜索表单存在");

  // 8. 验证操作按钮
  const detailButtons = page.locator("text=详情");
  const detailCount = await detailButtons.count();
  expect(detailCount).toBeGreaterThan(0);
  console.log(`✓ 详情按钮数: ${detailCount}`);

  // 9. 截图
  await page.screenshot({
    path: "screenshots/sft-work-doc-index.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-doc-index.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/doc/index`);
    await page.goto(`${BASE_URL}/sft/work/doc/index`);
