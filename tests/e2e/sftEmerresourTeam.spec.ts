 ...
 * 测试应急队伍台账页面
 */
async function testTeamPage(page: Page): Promise<void> {
  console.log("\n=== 测试应急队伍台账页面 ===");

  // 导航到应急队伍台账页面
  await page.goto(`${BASE_URL}/sft/emerresour/team/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("应急队伍台账");
  console.log("✓ 页面标题正确");
  ...
  await page.screenshot({
    path: "screenshots/sft-emerresour-team.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-emerresour-team.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/emerresour/team/index`);
    await page.goto(`${BASE_URL}/sft/emerresour/team/index`);
