 ...
 * 测试审验作业管理页面
 */
async function testFailPage(page: Page): Promise<void> {
  console.log("\n=== 测试审验作业管理页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/work/doc/fail`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("审验作业管理");
  console.log("✓ 页面标题正确");
  ...
  const examineBtn = page.locator('button:has-text("审验")');
  const deleteBtn = page.locator('button:has-text("删除")');
  await expect(detailBtn.first()).toBeVisible();
  await expect(examineBtn.first()).toBeVisible();
  ...
  await page.screenshot({
    path: "screenshots/sft-work-doc-fail.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-doc-fail.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/doc/fail`);
    await page.goto(`${BASE_URL}/sft/work/doc/fail`);
