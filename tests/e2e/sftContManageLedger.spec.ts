 ...
 * 测试承包商台账页面
 */
async function testLedgerPage(page: Page): Promise<void> {
  console.log("\n=== 测试承包商台账页面 ===");

  // 导航到台账页面
  await page.goto(`${BASE_URL}/sft/cont/manage/ledger/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("承包商台账");
  console.log("✓ 页面标题正确");
  ...
  // 8. 验证黑名单行样式
  const blacklistRow = page.locator(".blacklist-row");
  const blacklistCount = await blacklistRow.count();
  if (blacklistCount > 0) {
    console.log(`✓ 黑名单行数: ${blacklistCount}`);
  }

  // 9. 测试搜索功能
  await searchInput.first().fill("德州");
  ...
  await page.screenshot({
    path: "screenshots/sft-cont-manage-ledger.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-cont-manage-ledger.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/cont/manage/ledger/index`);
    await page.goto(`${BASE_URL}/sft/cont/manage/ledger/index`);
