 ...
 * 测试更换台账页面
 */
async function testClPage(page: Page): Promise<void> {
  console.log("\n=== 测试更换台账页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/insp/cl/index`);
  ...
  expect(headerText).toContain("原手机型号");
  expect(headerText).toContain("原手机序列号1");
  expect(headerText).toContain("更换原因");
  console.log("✓ 表头正确");
  ...
  // 5. 验证搜索框
  const searchInput = page.locator('input[placeholder*="部门"]');
  ...
  const delAllBtn = page.locator("button:has-text('删除')");
  ...
  expect(await delAllBtn.count()).toBeGreaterThan(0);
  console.log("✓ 功能按钮存在");

  // 8. 验证表格内操作按钮
  const editBtn = page.locator("button:has-text('编辑')");
  const deleteBtn = page.locator("button.el-button--danger:has-text('删除')");
  expect(await editBtn.count()).toBeGreaterThan(0);
  expect(await deleteBtn.count()).toBeGreaterThan(0);
  console.log("✓ 编辑/删除按钮存在");

  // 9. 测试搜索功能
  await searchInput.fill("PVC");
  ...
  await page.screenshot({
    path: "screenshots/sft-insp-cl.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-insp-cl.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/insp/cl/index`);
    await page.goto(`${BASE_URL}/sft/insp/cl/index`);
