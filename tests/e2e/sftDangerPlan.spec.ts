...
async function testPlanPage(page: Page): Promise<void> {
  ...
  // 导航到隐患排查计划页面
  await page.goto(`${BASE_URL}/sft/danger/plan/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("隐患排查任务");
  console.log("✓ 页面标题正确");
  ...
  console.log("表头列:", headers.filter(h => h.trim()).slice(0, 7));
  ...
  await page.screenshot({
    path: "screenshots/sft-danger-plan.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-danger-plan.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/danger/plan/index`);
    await page.goto(`${BASE_URL}/sft/danger/plan/index`);
