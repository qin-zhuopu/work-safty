 ...
 * 测试应急预案台账页面
 */
async function testEpPage(page: Page): Promise<void> {
  console.log("\n=== 测试应急预案台账页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/emerresour/ep/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("应急预案台账");
  console.log("✓ 页面标题正确");
  ...
  expect(headers.some(h => h.includes("预案名称"))).toBeTruthy();
  expect(headers.some(h => h.includes("级别"))).toBeTruthy();
  ...
  // 7. 验证搜索表单
  const searchInput = page.locator('input[placeholder*="预案名称"]');
  ...
  expect(buttonTexts.some(t => t.includes("查询"))).toBeTruthy();
  expect(buttonTexts.some(t => t.includes("重置"))).toBeTruthy();
  expect(buttonTexts.some(t => t.includes("添加"))).toBeTruthy();
  expect(buttonTexts.some(t => t.includes("删除"))).toBeTruthy();
  console.log("✓ 操作按钮存在");
  ...
  await page.screenshot({
    path: "screenshots/sft-emerresour-ep.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-emerresour-ep.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/emerresour/ep/index`);
    await page.goto(`${BASE_URL}/sft/emerresour/ep/index`);
