 ...
 * 测试特殊工管理页面
 */
async function testSpecialPage(page: Page): Promise<void> {
  console.log("\n=== 测试特殊工管理页面 ===");

  // 导航到特殊工管理页面
  await page.goto(`${BASE_URL}/sft/work/special/index`);
  ...
  // 6. 验证搜索表单存在
  const searchInput = page.locator('input[placeholder*="请输入姓名"]');
  await expect(searchInput.first()).toBeVisible();
  console.log("✓ 搜索表单已渲染");

  // 7. 验证特种作业类型下拉框
  const categorySelect = page.locator('.el-select:has-text("请选择")');
  await expect(categorySelect.first()).toBeVisible();
  console.log("✓ 特种作业类型下拉框已渲染");
  ...
  console.log(`✓ 第一行姓名: ${firstUserName}`);

  // 11. 截图
  await page.screenshot({
    path: "screenshots/sft-work-special.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-special.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/special/index`);
    await page.goto(`${BASE_URL}/sft/work/special/index`);
