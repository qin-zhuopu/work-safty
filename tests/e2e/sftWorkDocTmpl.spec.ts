 ...
 * 测试模板管理页面
 */
async function testTmplPage(page: Page): Promise<void> {
  console.log("\n=== 测试模板管理页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/work/doc/tmpl/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("模板管理");
  console.log("✓ 页面标题正确");
  ...
  // 5. 验证类型下拉选择框
  const typeLabel = page.locator(".el-form-item__label:has-text('类型')");
  ...
  console.log("✓ 检索/重置按钮存在");

  // 7. 验证表格内操作按钮
  const nodesBtn = page.locator("button:has-text('节点')");
  expect(await nodesBtn.count()).toBeGreaterThan(0);
  console.log("✓ 节点按钮存在");

  // 8. 测试类型筛选功能
  await typeSelect.click();
  await page.waitForTimeout(300);
  const fireOption = page.locator(
    '.el-select-dropdown__item:has-text("动火一级")'
  );
  if ((await fireOption.count()) > 0) {
    await fireOption.first().click();
    await searchBtn.click();
    await page.waitForTimeout(500);
    console.log("✓ 类型筛选功能测试完成");
  }
  ...
  await page.screenshot({
    path: "screenshots/sft-work-doc-tmpl.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-doc-tmpl.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/doc/tmpl/index`);
    await page.goto(`${BASE_URL}/sft/work/doc/tmpl/index`);
