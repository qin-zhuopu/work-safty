...
async function testDoccameraPage(page: Page): Promise<void> {
  ...
  // 导航到目标页面
  await page.goto(`${BASE_URL}/sft/work/doccamera/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("作业票证视频监控");
  console.log("✓ 页面标题正确");
  ...
  const playButtons = page.locator(
    '.el-table__body .el-button--primary:has-text("播放")'
  );
  const playbackButtons = page.locator(
    '.el-table__body .el-button--primary:has-text("回放")'
  );
  const playCount = await playButtons.count();
  const playbackCount = await playbackButtons.count();
  expect(playCount).toBeGreaterThan(0);
  expect(playbackCount).toBeGreaterThan(0);
  console.log(`✓ 播放按钮数: ${playCount}, 回放按钮数: ${playbackCount}`);
  ...
  await page.screenshot({
    path: "screenshots/sft-work-doccamera.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-work-doccamera.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/work/doccamera/index`);
    await page.goto(`${BASE_URL}/sft/work/doccamera/index`);
