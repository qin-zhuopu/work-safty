 ...
 * 测试部门管理页面
 */
async function testDeptPage(page: Page): Promise<void> {
  console.log("\n=== 测试部门管理页面 ===");

  // 导航到部门管理页面
  await page.goto(`${BASE_URL}/sft/sys/dept/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span");
  expect(title).toContain("部门结构");
  console.log("✓ 页面标题正确");
  ...
  console.log(`✓ 人员数据行数: ${dataRows}`);

  // 7. 验证操作按钮组
  const treeButtons = page.locator(".tree-actions .el-button");
  const buttonCount = await treeButtons.count();
  expect(buttonCount).toBe(4);
  console.log(`✓ 部门操作按钮数: ${buttonCount}`);

  // 8. 验证搜索工具栏
  const searchInput = page.locator('.toolbar input[placeholder*="姓名"]');
  await expect(searchInput.first()).toBeVisible();
  console.log("✓ 搜索输入框存在");

  // 9. 点击部门树节点
  const firstTreeNode = page.locator(".el-tree-node__content").first();
  ...
  await page.screenshot({
    path: "screenshots/sft-sys-dept-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/sft-sys-dept-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/sys/dept/index`);
    await page.goto(`${BASE_URL}/sft/sys/dept/index`);
