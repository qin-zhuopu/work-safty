 ...
 * 测试承包商人员审批页面
 */
async function testExamineperPage(page: Page): Promise<void> {
  console.log("\n=== 测试承包商人员审批页面 ===");

  // 导航到页面
  await page.goto(`${BASE_URL}/sft/cont/per/examineper/index`);
  await page.waitForLoadState("networkidle");

  // 1. 验证页面标题
  const title = await page.textContent(".card-header span, h1, h2");
  expect(title).toContain("承包商人员审批");
  console.log("✓ 页面标题正确");
  ...
  // 3. 验证搜索表单
  const searchInputs = page.locator('.search-form input[type="text"]');
  ...
  // 7. 验证审核节点标签
  const examineTags = page.locator(".el-table__body .el-tag");
  const tagCount = await examineTags.count();
  expect(tagCount).toBeGreaterThan(0);
  console.log(`✓ 审核节点标签数: ${tagCount}`);

  // 8. 验证操作按钮
  const actionButtons = page.locator(".el-button-group .el-button");
  const buttonCount = await actionButtons.first().count();
  expect(buttonCount).toBeGreaterThan(0);
  console.log(`✓ 操作按钮数: ${buttonCount}`);

  // 9. 测试搜索功能
  await page.fill('.search-form input[placeholder*="姓名"]', "刘");
  ...
  await page.screenshot({
    path: "screenshots/examineper-page.png",
    fullPage: true
  });
  console.log("✓ 已保存截图: screenshots/examineper-page.png");
...
async function runTests() {
  ...
  try {
    ...
    // 先打开目标页面
    console.log(`\n正在打开页面: ${BASE_URL}/sft/cont/per/examineper/index`);
    await page.goto(`${BASE_URL}/sft/cont/per/examineper/index`);
